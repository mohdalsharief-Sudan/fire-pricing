...
/* ================= تحويل العرض المعتمد إلى عقد/مشروع ================= */

async function exportApprovedQuote() {
  try {
    if (!state.project.name.trim()) { toast("أدخل اسم المشروع أولاً"); return; }
    if (!state.equipment.length && !state.materials.length) { toast("أضف بنوداً للمشروع أولاً"); return; }
    const c = calcFull();
    const client = CLIENTS.find(x => x.id === meta.clientId);
    const payload = {
      format: "fire-safety-quote-v1",
      exported_at: new Date().toISOString(),
      source: "fire-pricing",
      quote: { quoteNo: meta.quoteNo, status: meta.status, date: state.project.date, validity: num(state.project.validity) },
      client: client ? {
        name: client.name, contact_person: "", phone: client.phone || "", email: client.email || "",
        address: [client.city, client.notes && client.notes.includes("س.ت") ? client.notes : ""].filter(Boolean).join(" | "),
        notes: [client.cr_number ? "س.ت: " + client.cr_number : "", client.notes || ""].filter(Boolean).join("\n"),
        cr_number: client.cr_number || "", city: client.city || ""
      } : (state.project.client ? { name: state.project.client, contact_person: "", phone: "", email: "", address: "", notes: "", cr_number: "", city: state.project.location || "" } : null),
      project: { name: state.project.name, location: state.project.location, area: num(state.project.area), floors: num(state.project.floors), notes: state.project.notes },
      /* القيمة التعاقدية الفعلية = السعر بعد الخصم (وليس الإجمالي قبل الخصم) */
      contract: {
        value: c.afterDiscount, vat: c.vat, grand_total: c.grandTotal,
        currency: state.project.currency, discount_pct: state.margins.discountPct || 0,
        discount_amount: c.discount, quoteNo: meta.quoteNo,
        title: "توريد وتركيب أنظمة الحماية من الحرائق — " + state.project.name
      },
      items: [
        ...state.equipment.map(e => ({
          kind: "equipment", name: e.name, qty: e.qty, unit: "وحدة",
          supply_cost: num(e.supplyCost), install_cost: num(e.installCost),
          sale_price: c.baseCost > 0 ? Math.round((num(e.supplyCost) + num(e.installCost)) * (c.afterDiscount / c.baseCost) * 100) / 100 : num(e.supplyCost) + num(e.installCost),
          total: c.baseCost > 0 ? Math.round((num(e.supplyCost) + num(e.installCost)) * num(e.qty) * (c.afterDiscount / c.baseCost) * 100) / 100 : num(e.qty) * (num(e.supplyCost) + num(e.installCost))
        })),
        ...state.materials.map(m => ({
          kind: "material", name: m.name, qty: m.qty, unit: m.unit, unit_cost: num(m.unitCost),
          sale_price: c.baseCost > 0 ? Math.round(num(m.unitCost) * (c.afterDiscount / c.baseCost) * 100) / 100 : num(m.unitCost),
          total: c.baseCost > 0 ? Math.round(num(m.qty) * num(m.unitCost) * (c.afterDiscount / c.baseCost) * 100) / 100 : num(m.qty) * num(m.unitCost)
        })),
        ...state.labor.map(l => ({
          kind: "labor", name: l.name, workers: l.workers, days: l.days, daily_cost: l.dailyCost,
          total: c.baseCost > 0 ? Math.round(l.workers * l.days * l.dailyCost * (c.afterDiscount / c.baseCost) * 100) / 100 : l.workers * l.days * l.dailyCost
        })),
        ...state.services.map(s => ({
          kind: "service", name: s.name, service_type: s.type, service_value: s.value,
          amount: s.type === "pct" ? c.baseCost * (num(s.value) / 100) : num(s.value)
        }))
      ]
    };
    const res = await API.exportApprovedQuote(payload);
    if (res && !res.canceled) toast("تم تصدير العقد المعتمد — استورده في Fire-Engineer-AI");
...
on("btnExportApproved", "click", exportApprovedQuote);
